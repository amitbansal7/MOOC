...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointsMeans = points.par.groupBy(findClosest(_, means))
    means.par.map(mean => mean -> pointsMeans.getOrElse(mean, GenSeq())).toMap
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    if (converged(eta)(means, updated) == false) kMeans(points, updated, eta) else updated
