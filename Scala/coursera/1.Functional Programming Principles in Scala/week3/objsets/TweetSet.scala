...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = filterAcc(x => this.contains(x), that.filterAcc(y => that.contains(y), new Empty))
   ...
   */
    ...
    def most(t:Tweet):Tweet
    def mostRetweeted: Tweet = {
        most(new Tweet("t", "t", 0))
    }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def most(t:Tweet):Tweet ={
    if(t.retweets > elem.retweets) left.most(right.most(t))
    else left.most(right.most(elem))
  }
