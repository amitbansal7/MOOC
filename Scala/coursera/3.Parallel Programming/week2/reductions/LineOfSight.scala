...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    def solve(i:Int, m:Float):Unit = if(i < input.length){
      val agl = max(m, input(i) / i)
      output(i) = agl;
      solve(i+1, agl)
    }
    output(0) = 0
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (from until until)
      .foldLeft(angle(input(from), from))((x, y) => x max angle(input(y), y))
  }
  ...
  def angle(inp:Float, i:Int) = inp/i
}
